...
public class DatabaseManager {
    ...
    // Point to D:\Database and Config by default
    private static final String DB_FOLDER =
        "D:" + File.separator + "Database and Config";

    private static final String DB_FILE =
        DB_FOLDER + File.separator + "JavaAppERP.db";

    // Use db.url from config.properties if available, otherwise fallback
    private static final String DB_URL =
        ConfigManager.get("db.url", "jdbc:sqlite:" + DB_FILE);
    ...
    private static void seedAdministrator() {
        String sql = "INSERT INTO users(username,password,full_name,role,email,active) " +
            "SELECT 'admin','admin','Administrator','ADMIN','shailesh.rockstar007@yahoo.com',1 " +
